...
String::String(const char *cstr) : _buf(nullptr), _len(0), _cap(0) {
  if (cstr) {
    _copy(cstr, static_cast<unsigned int>(strlen(cstr)));
  }
}
String::String(const char *cstr, size_t n) : _buf(nullptr), _len(0), _cap(0) {
  if (cstr && n) {
    _copy(cstr, static_cast<unsigned int>(n));
  }
}
...
String &String::operator+=(float n) {
  concat(String(n, 2));
  return *this;
}
String &String::operator+=(double n) {
  concat(String(n, 2));
  return *this;
}
...
bool String::operator==(const String &rhs) const {
  ...
  if (!_buf || !rhs._buf) {
    return (_len == 0 && rhs._len == 0);
  }
  return memcmp(_buf, rhs._buf, _len) == 0;
}
bool String::operator!=(const String &rhs) const { return !(*this == rhs); }
bool String::operator<(const String &rhs) const { return strcmp(_buf ? _buf : "", rhs._buf ? rhs._buf : "") < 0; }
...
// Element access
// ---------------------------------------------------------------------------
char String::operator[](unsigned int i) const { return (_buf && i < _len) ? _buf[i] : '\0'; }
...
int String::indexOf(const String &str, unsigned int from) const {
  if (!_buf || !str._buf || from >= _len || str._len == 0) {
    return -1;
  }
  for (unsigned int i = from; i + str._len <= _len; i++) {
    if (memcmp(_buf + i, str._buf, str._len) == 0) {
...
int String::lastIndexOf(const String &str, unsigned int from) const {
  if (!_buf || !str._buf || str._len > _len) {
    return -1;
  }
  if (from >= _len) {
    from = _len - str._len;
...
bool String::equalsIgnoreCase(const String &s) const {
  ...
  for (unsigned int i = 0; i < _len; i++) {
    if (tolower(static_cast<unsigned char>(_buf[i])) != tolower(static_cast<unsigned char>(s._buf[i]))) {
      return false;
    }
  }
  return true;
...
void String::trim() {
  ...
  while (start < _len && isspace(static_cast<unsigned char>(_buf[start]))) {
    start++;
  }
  // trim trailing
  unsigned int end = _len;
  while (end > start && isspace(static_cast<unsigned char>(_buf[end - 1]))) {
    end--;
  }
  unsigned int newLen = end - start;
  if (start > 0) {
    memmove(_buf, _buf + start, newLen);
  }
  _len = newLen;
  _buf[_len] = '\0';
}
